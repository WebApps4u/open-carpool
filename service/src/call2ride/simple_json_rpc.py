#from http://code.activestate.com/recipes/552751/
...
try:
    import json
    json.dumps    ##on some systems there is a module callled json 
                  ##whitch is incompatible with the python2.6 standart json module simplejson is
except ImportError:
...
class SimpleJSONRPCServer(SocketServer.TCPServer,
    ...
    """
    logger.debug('3O3LIOLIOLI')
    allow_reuse_address = True

    def __init__(self, addr, requestHandler=SimpleJSONRPCRequestHandler,
                 logRequests=True):
        self.logRequests = logRequests
        logger.debug('__init__ addr: %s' % addr[0])
...
if __name__ == '__main__':
    if not len(sys.argv) > 1:
        import socket
        print 'Running JSON-RPC server on port 8001'
